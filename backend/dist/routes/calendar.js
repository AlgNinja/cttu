...
router.get('/', async (req, res) => {
    try {
        ...
        if (startDate || endDate) {
            todoWhere.dueDate = {};
            if (startDate)
                todoWhere.dueDate.gte = startDate;
            if (endDate)
                todoWhere.dueDate.lte = endDate;
        }
        // Fetch events
        const eventWhere = { clerkUserId };
        if (startDate || endDate) {
            eventWhere.AND = [];
            if (startDate)
                eventWhere.AND.push({ endDate: { gte: startDate } });
            if (endDate)
                eventWhere.AND.push({ startDate: { lte: endDate } });
        }
        // Fetch time chunks
        const chunkWhere = { clerkUserId };
        if (startDate || endDate) {
            chunkWhere.AND = [];
            if (startDate)
                chunkWhere.AND.push({ endTime: { gte: startDate } });
            if (endDate)
                chunkWhere.AND.push({ startTime: { lte: endDate } });
        }
        const [todos, events, timeChunks] = await Promise.all([
            prisma_js_1.default.todo.findMany({ where: todoWhere, orderBy: { dueDate: 'asc' } }),
            prisma_js_1.default.event.findMany({ where: eventWhere, orderBy: { startDate: 'asc' } }),
            prisma_js_1.default.timeChunk.findMany({ where: chunkWhere, orderBy: { startTime: 'asc' } }),
        ]);
        const unifiedItems = [];
